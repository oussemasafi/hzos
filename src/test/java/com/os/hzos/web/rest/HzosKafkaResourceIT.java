...
import com.os.hzos.config.KafkaProperties;
...
class HzosKafkaResourceIT {
    ...
    void setup() {
        ...
        HzosKafkaResource kafkaResource = new HzosKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/hzos-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/hzos-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
